...
def check_visualization_libraries() -> Dict[str, bool]:
    ...
    """
    return {
        'matplotlib': True,  # Always required
        'pyvista': HAS_PYVISTA,
        'vtk': HAS_VTK
    }
    ...
    **kwargs
    ...
    if isinstance(data, (str, Path)):
        ...
        if str(data).endswith('.vtu'):
            if not HAS_VTK:
                raise ImportError("VTK is required for reading VTU files. Install it with 'pip install vtk'.")
            mesh, _, _ = extract_mesh_from_vtu(data)
            data = mesh
        elif str(data).endswith('.vtp'):
            if not HAS_VTK:
                raise ImportError("VTK is required for reading VTP files. Install it with 'pip install vtk'.")
    ...
    if use_pyvista:
        if not HAS_PYVISTA:
            raise ImportError("PyVista is required for 3D visualization. Install it with 'pip install pyvista'.")
