...
def extract_mesh_from_vtu(
    vtu_file: str
) -> Tuple[Optional['vtk.vtkUnstructuredGrid'], Optional[Dict[str, np.ndarray]], Optional[Dict[str, np.ndarray]]]:
    """
...
def extract_points_from_vtp(
    vtp_file: str
) -> Tuple[Optional['vtk.vtkPolyData'], Optional[Dict[str, np.ndarray]], Optional[Dict[str, np.ndarray]]]:
    """
...
def extract_attributes(
    ...
    """
    if not HAS_VTK:
        raise ImportError("VTK is required for attribute extraction. Install it with 'pip install vtk'.")
...
def convert_vtk_to_numpy(
    ...
    """
    if not HAS_VTK:
        raise ImportError("VTK is required for array conversion. Install it with 'pip install vtk'.")
...
def save_as_vtu(
    ...
    """
    if not HAS_VTK:
        raise ImportError("VTK is required for saving VTU files. Install it with 'pip install vtk'.")
...
def save_as_vtp(
    ...
    """
    if not HAS_VTK:
        raise ImportError("VTK is required for saving VTP files. Install it with 'pip install vtk'.")
...
def merge_vtu_files(
    ...
    """
    if not HAS_VTK:
        raise ImportError("VTK is required for merging VTU files. Install it with 'pip install vtk'.")
